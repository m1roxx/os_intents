...
void main() {
  group('a plain intent', () {
    ...
    test('carries the id in the data URI', () {
      // Measured on a stock project: a data URI survives into the Intent the
      // system builds, which is what the runtime reads it back out of.
    ...
    test('points its labels at resources, since a literal is refused', () {
      expect(
        xml,
        contains(
          'android:shortcutShortLabel="@string/os_intents_addTask_label_short"',
        ),
      );
      expect(
        xml,
        contains(
          'android:shortcutLongLabel="@string/os_intents_addTask_label_long"',
        ),
      );
    });
  ...
  group('the strings that go with it', () {
    test('carry the title and the description', () {
      ...
      expect(
        xml,
        contains(
          '<string name="os_intents_addTask_label_short">Add task</string>',
        ),
      );
      expect(
        xml,
    ...
    test('fall back to the title when there is no description', () {
      expect(
        ...
        contains(
          '<string name="os_intents_addTask_label_long">Add task</string>',
        ),
      );
    });
  ...
  group('a capability', () {
    ...
    test('is declared, and bound from the shortcut', () {
      expect(
        xml,
        contains('<capability android:name="actions.intent.CREATE_TASK">'),
      );
      expect(
        xml,
        contains(
  ...
  group('what is left out', () {
    test(
      'androidShortcut: false drops the shortcut but keeps the capability',
      () {
        final xml = shortcuts(
          manifest([
            intent(
  ...
  group('the file itself', () {
    test('says it is generated', () {
      expect(
        shortcuts(manifest([intent()])),
        contains('GENERATED BY os_intents'),
      );
      expect(
        strings(manifest([intent()])),
        contains('GENERATED BY os_intents'),
      );
    });
    ...
    test('parses as XML, with the structure Android expects', () {
      ...
      expect(
        shortcut.findElements('intent').single.getAttribute('android:data'),
        'osintents://intent/addTask',
      );
      expect(
        shortcut
            .findElements('capability-binding')
            .single
